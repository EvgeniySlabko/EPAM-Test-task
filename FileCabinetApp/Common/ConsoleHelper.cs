...
namespace FileCabinetApp
{
    ...
    public static class ConsoleHelper
    {
        ...
        public static void EnterRecord(out FileCabinetRecord newRecord, ValidationSettings validationSettings)
        {
            ...
            newRecord.FirstName = ReadInput(new Converter().Convert<string>, new StringValidator(validationSettings.FirstName.Min, validationSettings.FirstName.Max).Validate);

            Console.Write(StringManager.Rm.GetString("LastNameMessage", CultureInfo.CurrentCulture));
            newRecord.LastName = ReadInput(new Converter().Convert<string>, new StringValidator(validationSettings.LastName.Min, validationSettings.LastName.Max).Validate);

            Console.Write(StringManager.Rm.GetString("DateOfBirthMessage", CultureInfo.CurrentCulture));
            newRecord.DateOfBirth = ReadInput(new Converter().Convert<DateTime>, new DateValidator(validationSettings.DateModel.From, validationSettings.DateModel.To).Validate);

            Console.Write(StringManager.Rm.GetString("IdentificationNumberMessage", CultureInfo.CurrentCulture));
            newRecord.IdentificationNumber = ReadInput(new Converter().Convert<decimal>, new DecimalValidator(validationSettings.IdentificationNumberModel.Min, validationSettings.IdentificationNumberModel.Max).Validate);

            Console.Write(StringManager.Rm.GetString("IdentificationLetterMessage", CultureInfo.CurrentCulture));
            newRecord.IdentificationLetter = ReadInput(new Converter().Convert<char>, new CharValidator(c => char.IsLetter(c)).Validate);

            Console.Write(StringManager.Rm.GetString("PointsForFourTestsMessage", CultureInfo.CurrentCulture));
            newRecord.PointsForFourTests = ReadInput(new Converter().Convert<short>, new ShortValidator(validationSettings.PointsModel.Min, validationSettings.PointsModel.Max).Validate);
        }
